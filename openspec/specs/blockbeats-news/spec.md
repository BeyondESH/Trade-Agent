...
系统 SHALL 通过后端代理接入 BlockBeats 快讯 API(`api-pro.theblockbeats.info`),后端新增 `GET /api/blockbeats/newsflash/{type}` 代理路由,API key 从 `backend/.env` 的 `BB_API_KEY` 读取,key SHALL 永不暴露给浏览器。前端只调用自家 `/api/blockbeats/newsflash/*`。
