...
系统 SHALL 支持当用户向图表左侧翻页越过本地已存最早 bar 时，按需触发对应 symbol/timeframe/category 的历史深度回灌：后端通过 v3 history-candles 通道向后分页拉取更早区间并落库，随后 `/candles` MUST 能连续返回这些更早区间的数据。回灌 MUST 回溯至交易所真实最早历史；`earliest_reached` 判定 SHALL 仅基于可无限回溯通道（v3）的最旧窗口空页（重试一次后）。
...
- **WHEN** 可无限回溯通道（v3）对最旧窗口返回空页（重试一次后）
- **THEN** 系统 SHALL 停止继续回灌并向前端标识"已到最早"
- **AND** 不进入无限循环
- **AND** 受近端窗口深度限制的通道返回空页时 MUST NOT 触发终止
