# klinecharts-pro-integration Specification (Delta)

## MODIFIED Requirements

### Requirement: Pro 图表终端集成
系统 SHALL 基于 @klinecharts/pro(clone 至项目 vendor 本地)渲染交易所式图表终端;Pro 的内置绘图工具条、周期条、指标管理、标的搜索等 chrome SHALL 被禁用,改为由模板工具栏作为唯一指挥层调用 Pro 底层 API;数据通过 datafeed 契约接入后端。

#### Scenario: 渲染 Pro 终端
- **WHEN** React 页面挂载 KLineChartPro 包装器
- **THEN** SHALL 渲染出包含绘图工具条/周期条/指标管理的图表终端
- **AND** 渲染的 chrome 由模板外壳提供(绘图工具栏在左侧、周期在顶部),Pro 自带的绘图工具条/周期条/指标弹窗/搜索弹窗 SHALL 隐藏
...
- **THEN** SHALL 返回底层 klinecharts 实例(用于自动化/序列化)
