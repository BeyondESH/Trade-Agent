...
## Purpose
独立「回测」tab 合并进 QUANT LAB 面板,开单列表、收益图形、历史回看迁移为面板内 Tabs,并补 sortable 表格与完整指标展示。
...
QUANT LAB 的「曲线分析」视图 SHALL 使用 Recharts 渲染收益相关图形:月度收益柱状图、单笔交易盈亏柱状图、收益分布直方图、权益与回撤曲线,并补充模型概率(proba)分布直方图。图形数据 SHALL 由可单测的纯函数计算。
