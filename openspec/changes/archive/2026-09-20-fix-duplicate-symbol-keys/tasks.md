...
- [x] 3.3 [已核验：Playwright 真机运行，watchlist/命令面板数百 symbol 无重复，控制台零 duplicate-key 警告] 启动前端连接后端，确认控制台不再出现 `Encountered two children with the same key` 警告，且 watchlist/agent 下拉无重复币种（需真实后端 + 双品类实时数据；数据层已由集成测试覆盖）
