...
from market_data.streamhub import PONG_FRAME, MarketStream, RefCountSubscription
...
def test_orderbook_snapshot_then_update() -> None:
    ...
    asyncio.run(
        s._handle_frame(
            None,
            _book_row([["101", "6"], ["102", "0"]], [["100", "0"]], 11, pseq=10, action="update"),
            CAT,
        )
    )
    book = s.orderbook("BTCUSDT")
...
def test_orderbook_seq_gap_resubscribes() -> None:
    ...
    asyncio.run(s._handle_frame(None, _book_row([["101", "5"]], [["100", "4"]], 10), CAT))
    s._ws = {CAT: _Ws()}  # inject a live connection so the resubscribe request is sent
    asyncio.run(
        s._handle_frame(None, _book_row([["102", "3"]], [], 20, pseq=99, action="update"), CAT)
    )
        ...
        frame = json.dumps(
            {
                ...
                "data": [
                    {"instId": "BTCUSDT", "price": str(i), "size": "1", "side": "buy", "ts": str(i)}
                ],
            }
        )
        asyncio.run(s._handle_frame(None, frame, CAT))
            ...
            json.dumps(
                {
                    ...
                    "arg": {"instType": CAT, "channel": "trade", "instId": "BTCUSDT"},
                    "data": [
                        {"instId": "BTCUSDT", "price": "1", "size": "1", "side": "buy", "ts": "1"}
                    ],
                }
            ),
            CAT,
            ...
            json.dumps(
                {
                    ...
                    "data": [{"instId": "BTCUSDT", "lastPr": "100.0"}],
                }
            ),
            CAT,
        )
            ...
            json.dumps(
                {
                    ...
                    "data": [{"instId": "BTCUSDT", "lastPr": "101.0"}],
                }
            ),
            SPOT,
        )
...
def test_ticker_refresh_once_emits_changes() -> None:
    ...
    rows = [
        {"instId": "BTCUSDT", "lastPr": "64100", "change24h": "1"},
