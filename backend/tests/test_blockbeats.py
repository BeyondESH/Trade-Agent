...
def _client() -> TestClient:
    return TestClient(create_app(Settings(data_dir=Path(tempfile.mkdtemp()))), raise_server_exceptions=False)
...
def test_data_proxy_forwards_with_api_key(monkeypatch) -> None:
    ...
    resp = _client().get("/blockbeats/data/dxy")
...
def test_data_proxy_network_param(monkeypatch) -> None:
    ...
    resp = _client().get("/blockbeats/data/top10_netflow", params={"network": "solana"})
    assert resp.status_code == 200
...
def test_data_proxy_unknown_endpoint_400(monkeypatch) -> None:
    monkeypatch.setenv("BB_API_KEY", "test-key-123")
    resp = _client().get("/blockbeats/data/not_an_endpoint")
...
def test_newsflash_proxy_whitelist_and_forward(monkeypatch) -> None:
    ...
    resp = _client().get("/blockbeats/newsflash/ai", params={"page": 1, "size": 10, "lang": "cn"})
    assert resp.status_code == 200
...
def test_newsflash_proxy_unknown_type_400(monkeypatch) -> None:
    monkeypatch.setenv("BB_API_KEY", "test-key-123")
    resp = _client().get("/blockbeats/newsflash/bogus")
    assert resp.status_code == 400
