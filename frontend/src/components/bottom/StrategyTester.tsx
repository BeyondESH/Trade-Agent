      ...
      <div className="flex-1 overflow-y-auto p-3">
        {tab === "overview" ? (
          <div className="flex flex-col gap-4">
            ...
            <div
              className={`p-3 rounded-lg border flex flex-col gap-2 ${isDark ? "bg-[#1e222d] border-[#2a2e39]" : "bg-[#f8fafc] border-[#e0e3eb]"}`}
            >
              ...
              <div className="h-32 w-full flex items-end gap-1 pt-2">
                {result.equityCurve.map((pt, idx) => {
                  ...
                  const heightPct = Math.max(
                    10,
                    Math.min(100, ((pt.equity - minEq) / (maxEq - minEq)) * 100),
                  );
                  return (
                    <div
                      key={idx}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left font-mono text-[11px]">
              <thead>
                <tr
                  className={`border-b text-gray-500 uppercase text-[10px] ${isDark ? "border-[#2a2e39]" : "border-[#e0e3eb]"}`}
                >
              ...
              <tbody className="divide-y divide-gray-500/10">
                {result.trades.map((t) => {
                  const isWin = t.pnl > 0;
                  return (
                    <tr key={t.id} className={isDark ? "hover:bg-[#1e222d]" : "hover:bg-gray-50"}>
                      ...
                      <td
                        className={`py-1.5 px-2 font-bold ${isWin ? "text-[#089981]" : "text-[#f23645]"}`}
                      >
