import { Eye, Filter, Flame, Layers, TrendingDown, TrendingUp } from "lucide-react";
...
import { SymbolInfo, type ThemeMode } from "../../types/trading";
...
export const HeatmapsView: React.FC<Props> = ({ onOpenChartWithTicker, theme }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex-1 min-h-[450px] p-3 rounded-xl border flex flex-col gap-3 ${
          ...
          netflowLoading ? (
            ...
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 auto-rows-fr">
              {netflowRows.map((r) => {
                const size = Math.max(
                  18,
                  Math.round(Math.sqrt(Math.abs(r.netflow) / maxAbsNetflow) * 45),
                );
                return (
                  <div
                    key={r.symbol}
