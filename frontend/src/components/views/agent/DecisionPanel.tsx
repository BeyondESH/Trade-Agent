...
import { btnCls, Field, fmtNum, Panel, selectCls } from "./ui";
...
export const DecisionPanel: React.FC<Props> = ({
  ...
  return (
    ...
    >
      ...
      {!decision && !error && (
        <span className="text-xs text-gray-500">请求一次结构化开/平仓建议,不会产生任何下单。</span>
      )}
      {decision && (
          ...
          <div className="flex items-center gap-3">
            ...
            {decision.reference_price != null && (
              <span className="font-mono text-gray-400">
                参考价 {fmtNum(decision.reference_price)}
              </span>
            )}
            <span className="font-mono text-xs text-gray-400">
              置信 {fmtNum(decision.confidence * 100, 1)}%
            </span>
          </div>
