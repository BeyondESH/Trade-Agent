...
export const FactorIcChart: React.FC<Props> = ({ series = [], theme }) => {
  if (series.length < 2) {
    return (
      <div className="text-[11px] text-gray-400 py-2">
        暂无逐期 IC 时序数据 — 当前仅展示聚合 IC 排序;按时间窗口的 IC 趋势将在后端支持逐期 IC
        后提供。
      </div>
    );
  }
  ...
  return (
    <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="w-full" style={{ height: 120 }}>
      {factors.map((f, fi) => {
        const pts = series
          .filter((p) => p.factor === f)
          .map((p) => `${(p.bucket / 100) * 100},${20 - p.ic * 20 * (theme === "dark" ? 1 : 1)}`)
          .join(" ");
        return (
          <g key={f}>
            <polyline
