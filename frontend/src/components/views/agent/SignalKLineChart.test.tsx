...
vi.mock("../../chart/KLineChartProView", () => ({
  KLineChartProView: ({ onReady }: { onReady?: (chart: Chart | null) => void }) => {
...
describe("SignalKLineChart", () => {
  ...
  it("loads candles and draws overlays when a result has signals", async () => {
    ...
    await waitFor(() =>
      expect(chartMock.removeOverlay).toHaveBeenCalledWith({
  ...
  it("clears overlays but skips drawing when signal lane is empty", async () => {
    const empty = {
      ...result,
      series: { ...result.series, signal: [0, 0, 0] },
    };
    render(
      <SignalKLineChart
        symbol="BTCUSDT"
