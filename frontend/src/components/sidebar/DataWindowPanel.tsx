...
export const DataWindowPanel: React.FC<Props> = ({ symbol, activeCandle, indicators, theme }) => {
  ...
  return (
    <div id="data-window-panel" className="flex flex-col h-full w-full select-none text-xs">
      ...
      <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 font-mono text-xs">
        {/* Current Bar Info */}
        <div
          className={`p-2.5 rounded-lg border flex flex-col gap-1.5 ${isDark ? "bg-[#1e222d] border-[#2a2e39]" : "bg-[#f8fafc] border-[#e0e3eb]"}`}
        >
          ...
          {activeCandle ? (
            <>
              ...
              <div className="flex justify-between">
                ...
                <span
                  className={
                    activeCandle.close >= activeCandle.open ? "text-[#089981]" : "text-[#f23645]"
                  }
                >
        ...
        {/* Indicators Readings */}
        <div
          className={`p-2.5 rounded-lg border flex flex-col gap-2 ${isDark ? "bg-[#1e222d] border-[#2a2e39]" : "bg-[#f8fafc] border-[#e0e3eb]"}`}
        >
          <div className="font-sans font-bold text-[11px] text-gray-400 uppercase tracking-wider mb-1">
            Technical Plots
          </div>
        ...
        >
          ...
          {!pulseLoading && pulse.length === 0 && (
            <div className="text-gray-500 font-sans py-1">
              {t("Unavailable — check BB_API_KEY")}
            </div>
          )}
          {pulse.map((p) => (
            <div key={p.endpoint} className="flex items-center gap-2 py-0.5">
              <div className="flex-1 min-w-0">
                <div className="font-sans font-medium text-[11px] text-gray-400 truncate">
                  {p.label}
                </div>
